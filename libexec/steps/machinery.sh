    ...
    case "$1" in
        ...
        "install")
            if ! [ -x "$(command -v "machinery")" ]; then
                ...
                case "$lsb_dist" in
                    alpine)
                        primer_packages add tcl tclx tcl-tls tcllib
                        ;;
                    clear*linux*)
                        primer_packages add tcl-basic
                        ;;
                    ubuntu|*bian)
                        primer_packages add tcl tclx tcl-tls tcllib tcllib-critcl tcl-vfs
                        ;;
                    *)
                        ;;
                esac
                primer_dependency git
                yush_info "Installing machinery from $MACHINERY_REPO (branch: $MACHINERY_BRANCH)"
                $PRIMER_SUDO mkdir -p "${PRIMER_OPTDIR%%/}/machinery/$MACHINERY_BRANCH"
                $PRIMER_SUDO git clone "$MACHINERY_REPO" \
                                --recurse \
                                --branch "$MACHINERY_BRANCH" \
                                --depth 1 \
                                "${PRIMER_OPTDIR%%/}/machinery/$MACHINERY_BRANCH"
                yush_debug "Installing as ${PRIMER_BINDIR%%/}/machinery"
                $PRIMER_SUDO chmod a+x "${PRIMER_OPTDIR%%/}/machinery/$MACHINERY_BRANCH/machinery"
                $PRIMER_SUDO ln -s "${PRIMER_OPTDIR%%/}/machinery/$MACHINERY_BRANCH/machinery" "${PRIMER_BINDIR%%/}/machinery"
